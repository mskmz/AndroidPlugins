...
import static com.mskmz.main.BuildConfig.DEBUG;

public class PluginManager2 {

  //---------------DEBUG配置---------------------------------------------------------------------------
  private static final String TAG = "PluginManager2>>>";
  private static final boolean DEBUG = true;

  private static PluginManager2 pluginManager;
  ...
  public static PluginManager2 getInstance(Context context) {
    if (pluginManager == null) {
      synchronized (PluginManager.class) {
        if (pluginManager == null) {
          pluginManager = new PluginManager2(context);
        }
      }
    }
  ...
  public PluginManager2(Context context) {
  ...
  public void loadPlugin(File file) {
    try {
      if (DEBUG) Log.d(TAG, "parserApkAction: " + file);
      if (!file.exists()) {
        Log.d(TAG, "插件包 不存在...");
        return;
      }
